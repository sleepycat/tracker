...
const sendPasswordResetLink = (i18n) =>
  new mutationWithClientMutationId({
    name: 'SendPasswordResetLink',
    description: i18n._(
      t`This mutation allows a user to provide their username and request that a password reset email be sent to their account with a reset token in a url.`,
    ),
    inputFields: () => ({
      userName: {
        type: GraphQLNonNull(EmailAddress(i18n)),
        description: i18n._(
          t`User name for the account you would like to receive a password reset link for.`,
        ),
      },
    }),
    outputFields: () => ({
      status: {
        ...
        description: i18n._(
          t`Informs the user if the password reset email was sent successfully.`,
        ),
